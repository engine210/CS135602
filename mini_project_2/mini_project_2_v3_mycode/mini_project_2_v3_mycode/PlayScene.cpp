...
void PlayScene::OnKeyDown(int keyCode) {
	...
	// TODO_DONE 2 (5/8): Make the R key to create the 4th turret.
...
void PlayScene::ConstructUI() {
	...
	UIGroup->AddNewControlObject(btn);
	// TODO_DONE 2 (3/8): Create a button to support constructing the 4th tower.
    btn = new TurretButton("play/floor.png", "play/dirt.png",
        Engine::Sprite("play/tower-base.png", 1522, 136, 0, 0, 0, 0),
        Engine::Sprite("play/turret-6.png", 1522, 136, 0, 0, 0, 0)
        , 1522, 136, StickyTurret::Price);
    btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 3));
    UIGroup->AddNewControlObject(btn);
    
    //slider bar
    Slider *sliderBGM, *sliderSFX;
    sliderBGM = new Slider(1390, 240, 190, 4);
    sliderBGM->SetOnValueChangedCallback(std::bind(&PlayScene::BGMSlideOnValueChanged, this, std::placeholders::_1));
    AddNewControlObject(sliderBGM);
    AddNewObject(new Engine::Label("BGM: ", "pirulen.ttf", 20, 1340, 240, 0, 0, 0, 255, 0.5, 0.5));
    sliderSFX = new Slider(1390, 290, 190, 4);
    sliderSFX->SetOnValueChangedCallback(std::bind(&PlayScene::SFXSlideOnValueChanged, this, std::placeholders::_1));
    AddNewControlObject(sliderSFX);
    AddNewObject(new Engine::Label("SFX: ", "pirulen.ttf", 20, 1340, 290, 0, 0, 0, 255, 0.5, 0.5));
    bgmInstance = al_create_sample_instance(Engine::Resources::GetInstance().GetSample("play.ogg").get());
    ...
    // Mute button
    /*
    Engine::ImageButton* muteBtn;
    muteBtn = new Engine::ImageButton("stage-select/dirt.png", "stage-select/floor.png", 1522, 350, 0, 0);
    muteBtn->SetOnClickCallback(std::bind(&PlayScene::MuteOnClick, this));
    AddNewControlObject(muteBtn);
    */
    muteIcon = new Engine::StaticImageButton("play/unmute.png", "play/unmute.png", 1440, 350, 65, 65);
    muteIcon->SetOnClickCallback(std::bind(&PlayScene::MuteOnClick, this));
    AddNewControlObject(muteIcon);
    imgMute = Engine::Resources::GetInstance().GetBitmap("play/mute.png");
    imgUnmute = Engine::Resources::GetInstance().GetBitmap("play/unmute.png");
    
    // Exit button
    Engine::StaticImageButton* exitBtn;
    exitBtn = new Engine::StaticImageButton("play/exit.png", "play/exit.png", 1522, 355, 65, 65);
    exitBtn->SetOnClickCallback(std::bind(&PlayScene::ExitOnClick, this));
    AddNewControlObject(exitBtn);
}
...
void PlayScene::SFXSlideOnValueChanged(float value) {
 ...
 // Mute button call back
void PlayScene::MuteOnClick() {
    if (is_mute) {
        BGMSlideOnValueChanged(BGMv);
        SFXSlideOnValueChanged(SFXv);
        is_mute = false;
        muteIcon->bmp = imgUnmute;
    }
    else {
        BGMv = AudioHelper::BGMVolume;
        SFXv = AudioHelper::SFXVolume;
        BGMSlideOnValueChanged(0);
        SFXSlideOnValueChanged(0);
        is_mute = true;
        muteIcon->bmp = imgMute;
    }
}
...
void PlayScene::UIBtnClicked(int id) {
    ...
    else if (id == 3 && money >= StickyTurret::Price)
        preview = new StickyTurret(0, 0);
    // TODO_DONE 2 (4/8): On callback, create the 4th tower.
    if (!preview)
		return;
	preview->Position = Engine::GameEngine::GetInstance().GetMousePosition();
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
	...
	while (!que.empty()) {
		...
		que.pop();
		// TODO_DONE 3 (1/1): Implement a BFS starting from the most right-bottom block in the map.
		//               For each step you should assign the corresponding distance to the most right-bottom block.
		//               mapState[y][x] is TILE_DIRT if it is empty.
        
        //std::cout << "yx is " << p.y << " " << p.x << std::endl;
